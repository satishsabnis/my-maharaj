...
export default function MealWizardScreen() {
  const params = useLocalSearchParams<{ festivalName?: string; festivalDate?: string; autoStart?: string }>();
  const [step, setStep] = useState<WizardStep>('period');
  const [error, setError] = useState('');
  const [festivalContext, setFestivalContext] = useState('');

  // Handle festival params on mount
  useEffect(() => {
    if (params.festivalName && params.festivalDate && params.autoStart === 'true') {
      const fd = startOfDay(new Date(params.festivalDate));
      setSelectedFrom(fd);
      setSelectedTo(fd);
      setNumDays(1);
      setPlanMode('custom' as any);
      setFestivalContext(`Today is ${params.festivalName}. Generate traditional ${params.festivalName} festival dishes appropriate for this occasion. Prioritise fasting-friendly options if relevant.`);
      setStep('food-pref');
    }
  }, []);
  ...
  const [planMode,     setPlanMode]     = useState<'custom'|'week'>('custom');
  const [startDayOffset, setStartDayOffset] = useState(0);
  const [numDays,      setNumDays]      = useState(3);
  const [weekMood,     setWeekMood]     = useState<'balanced'|'light'|'cheat'|'festival'>('balanced');
  ...
  const [skippedSlots, setSkippedSlots] = useState<Record<number, MealSlotKey[]>>({});

  function isSkipped(dayIdx: number, slot: MealSlotKey): boolean { return (skippedSlots[dayIdx] ?? []).includes(slot); }
  function toggleSkip(dayIdx: number, slot: MealSlotKey) {
    setSkippedSlots(prev => { const cur = prev[dayIdx] ?? []; return { ...prev, [dayIdx]: cur.includes(slot) ? cur.filter(s => s !== slot) : [...cur, slot] }; });
  }

  // Load today's regen count on mount
  useEffect(() => {
    const key = `regenerate_count_${new Date().toISOString().split('T')[0]}`;
    AsyncStorage.getItem(key).then(v => { if (v) setRegenCount(parseInt(v, 10)); });
  }, []);
  ...
  async function tryRegenerate() {
    ...
    if (current >= 5) {
      Alert.alert('Daily limit reached', 'You can regenerate up to 5 times per day. Try again tomorrow.');
      return false;
    }
  ...
  const runGeneration = useCallback(async () => {
    ...
    try {
      ...
      const plan = await generateMealPlan({
        ...
        nutritionFocus: [
          weekMood === 'balanced' ? 'Nutritionally balanced meals across all food groups' :
          weekMood === 'light' ? 'Light, easy-to-digest meals. No heavy curries or fried food.' :
          weekMood === 'cheat' ? 'Rich, indulgent, comfort food. Biryani, fried snacks, sweets allowed.' :
          weekMood === 'festival' ? 'Traditional Indian festival dishes appropriate for the current season.' : '',
          nutritionGoals.length > 0 ? nutritionGoals.join(', ') : '',
          `Vary dishes (seed:${Date.now()})`,
        ].filter(Boolean).join('. '),
  ...
  }, [selectedFrom, selectedTo, foodPref, vegType, nonVegOpts, familyMembers, unwellIds, nutritionGoals, weekMood, bfPrefs, lnPrefs, dnPrefs, snPrefs, includeDessert, hasGuests, guestCuisine, guestDays, extraCuisines, perDayCuisine]);
  ...
  function renderPeriod() {
    ...
    const startDate = addDays(today, startDayOffset);
    const endDate = addDays(startDate, numDays - 1);
    const grocDay = session.groceryDay || 'Saturday';
    const grocDayIdx = WEEKDAYS.indexOf(grocDay);
    const todayIdx = today.getDay();
    const daysUntilGroc = (grocDayIdx - todayIdx + 7) % 7 || 7;
    const weekStart = addDays(today, daysUntilGroc);
    const weekEnd = addDays(weekStart, 6);

    function fmtShort(d: Date) { return `${WEEKDAYS[d.getDay()].substring(0,3)} ${d.getDate()}-${MONTHS[d.getMonth()]}`; }

    function handleNext() {
      if (planMode === 'custom') {
        setSelectedFrom(startDate); setSelectedTo(endDate);
      } else {
        setSelectedFrom(weekStart); setSelectedTo(weekEnd);
      }
      advance('food-pref');
    }
    ...
    return (
      <View>
        <Text style={s.stepTitle}>Plan with Maharaj</Text>

        {/* Mode tabs */}
        <View style={{flexDirection:'row',gap:8,marginBottom:16}}>
          {(['custom','week'] as const).map(m => (
            <TouchableOpacity key={m} style={{flex:1,paddingVertical:12,borderRadius:12,backgroundColor:planMode===m?navy:'rgba(255,255,255,0.92)',borderWidth:1.5,borderColor:planMode===m?navy:'rgba(27,58,92,0.12)',alignItems:'center'}} onPress={()=>setPlanMode(m)} activeOpacity={0.8}>
              <Text style={{fontSize:14,fontWeight:'700',color:planMode===m?white:'#1B3A5C'}}>{m==='custom'?'Custom Plan':'My Week'}</Text>
            </TouchableOpacity>
          ))}
        </View>

        {planMode === 'custom' ? (
          <View style={{backgroundColor:'rgba(255,255,255,0.95)',borderRadius:14,padding:16,borderWidth:1,borderColor:'rgba(27,58,92,0.1)'}}>
            {/* Start from stepper */}
            <Text style={{fontSize:12,fontWeight:'700',color:'#5A7A8A',textTransform:'uppercase',letterSpacing:0.5,marginBottom:8}}>Start from</Text>
            ...
            <Text style={{fontSize:12,fontWeight:'700',color:'#5A7A8A',textTransform:'uppercase',letterSpacing:0.5,marginBottom:8}}>Number of days</Text>
            <View style={{flexDirection:'row',alignItems:'center',justifyContent:'center',gap:16,marginBottom:16}}>
              <TouchableOpacity style={{width:40,height:40,borderRadius:20,backgroundColor:numDays>1?navy:'#D1D5DB',alignItems:'center',justifyContent:'center'}} onPress={()=>setNumDays(Math.max(1,numDays-1))} disabled={numDays<=1}>
                <Text style={{color:white,fontSize:20,fontWeight:'700'}}>-</Text>
              </TouchableOpacity>
              <Text style={{fontSize:36,fontWeight:'900',color:navy,minWidth:50,textAlign:'center'}}>{numDays}</Text>
              <TouchableOpacity style={{width:40,height:40,borderRadius:20,backgroundColor:numDays<7?navy:'#D1D5DB',alignItems:'center',justifyContent:'center'}} onPress={()=>setNumDays(Math.min(7,numDays+1))} disabled={numDays>=7}>
                <Text style={{color:white,fontSize:20,fontWeight:'700'}}>+</Text>
              </TouchableOpacity>
            </View>
            ...
            {/* Summary */}
            <Text style={{fontSize:13,color:'#5A7A8A',textAlign:'center',lineHeight:20}}>
              {numDays} day{numDays>1?'s':''} {'\u00B7'} {fmtShort(startDate)} to {fmtShort(endDate)}
            </Text>
            <Text style={{fontSize:11,color:'#9CA3AF',textAlign:'center',marginTop:4}}>Maharaj will plan breakfast, lunch, snack and dinner for each day</Text>
          </View>
        ) : (
          <View style={{backgroundColor:'rgba(255,255,255,0.95)',borderRadius:14,padding:16,borderWidth:1,borderColor:'rgba(27,58,92,0.1)'}}>
            <Text style={{fontSize:18,fontWeight:'800',color:navy,marginBottom:4}}>{grocDay} Maharaj</Text>
            <Text style={{fontSize:13,color:'#5A7A8A',lineHeight:20,marginBottom:12}}>Your grocery day is {grocDay}. Maharaj will plan 7 days starting this {grocDay}.</Text>
            <View style={{backgroundColor:'rgba(27,58,92,0.06)',borderRadius:10,paddingVertical:10,paddingHorizontal:14,marginBottom:12}}>
              <Text style={{fontSize:13,fontWeight:'600',color:navy,textAlign:'center'}}>{fmtShort(weekStart)} {'\u2192'} {fmtShort(weekEnd)} {'\u00B7'} 7 days</Text>
            </View>
            <Text style={{fontSize:11,color:'#9CA3AF',textAlign:'center'}}>Full week plan {'\u00B7'} PDF emailed on {grocDay} morning</Text>
            <TouchableOpacity style={{marginTop:12,alignItems:'center'}} onPress={()=>router.push('/dietary-profile' as never)}>
              <Text style={{fontSize:12,color:gold,fontWeight:'600'}}>Update grocery day in Family Profile</Text>
            </TouchableOpacity>
          </View>
        )}
  ...
  function renderFoodPref() {
    const GUEST_CUISINES = ['North Indian','South Indian','Punjabi','Bengali','Gujarati','Continental','Chinese','Arabic'];
    return (
      <View>
        ...
        {/* SECTION 1 — Food Type */}
        <Text style={s.sectionLabel}>FOOD TYPE</Text>
        ...
        {/* Inline protein chips */}
        {(foodPref === 'nonveg' || isMixed) && (
          <View style={{marginTop:8,marginBottom:4}}>
            <Text style={{fontSize:11,color:'#5A7A8A',marginBottom:6}}>Select proteins for this plan</Text>
            <View style={{flexDirection:'row',gap:8,flexWrap:'wrap'}}>
              {['Eggs','Chicken','Fish','Mutton'].map(o => (
                <TouchableOpacity key={o} style={{paddingHorizontal:14,paddingVertical:8,borderRadius:20,borderWidth:1.5,borderColor:nonVegOpts.includes(o)?'#C9A227':'#D1D5DB',backgroundColor:nonVegOpts.includes(o)?'#C9A227':'rgba(255,255,255,0.9)'}} onPress={()=>setNonVegOpts(prev=>prev.includes(o)?prev.filter(x=>x!==o):[...prev,o])} activeOpacity={0.8}>
                  <Text style={{fontSize:13,fontWeight:'600',color:nonVegOpts.includes(o)?'#1B2A0C':'#1B3A5C'}}>{o}</Text>
                </TouchableOpacity>
              ))}
            </View>
            {nonVegOpts.length === 0 && <Text style={s.inlineError}>Please select at least one protein</Text>}
          </View>
        )}

        {/* Veg sub-options */}
        {foodPref === 'veg' && !isMixed && (
          <View style={{flexDirection:'row',gap:8,marginTop:8,marginBottom:4}}>
            {[{l:'Normal Veg',v:'normal' as const},{l:'Fasting/Upvas',v:'fasting' as const}].map(({l,v})=>(
              <TouchableOpacity key={v} style={{paddingHorizontal:14,paddingVertical:8,borderRadius:20,borderWidth:1.5,borderColor:vegType===v?'#C9A227':'#D1D5DB',backgroundColor:vegType===v?'#C9A227':'rgba(255,255,255,0.9)'}} onPress={()=>setVegType(v)} activeOpacity={0.8}>
                <Text style={{fontSize:13,fontWeight:'600',color:vegType===v?'#1B2A0C':'#1B3A5C'}}>{l}</Text>
              </TouchableOpacity>
            ))}
          </View>
        )}
        ...
        {/* SECTION 2 — Meals to Plan */}
        <Text style={s.sectionLabel}>WHICH MEALS TO PLAN?</Text>
        <View style={{flexDirection:'row',flexWrap:'wrap',gap:8,marginBottom:12}}>
          {[{k:'breakfast',l:'Breakfast'},{k:'lunch',l:'Lunch'},{k:'snack',l:'Evening Snack'},{k:'dinner',l:'Dinner'}].map(({k,l})=>(
            <TouchableOpacity key={k}
              style={{paddingHorizontal:14,paddingVertical:8,borderRadius:20,borderWidth:1.5,borderColor:selectedSlots.includes(k)?'#1B3A5C':'#D4EDE5',backgroundColor:selectedSlots.includes(k)?'#1B3A5C':'rgba(255,255,255,0.9)'}}
              onPress={()=>setSelectedSlots(prev=>prev.includes(k)?prev.filter(x=>x!==k):[...prev,k])}>
              <Text style={{fontSize:13,fontWeight:'600',color:selectedSlots.includes(k)?'#FFFFFF':'#1B3A5C'}}>{l}</Text>
            </TouchableOpacity>
          ))}
        </View>
        ...
        <View style={{flexDirection:'row',gap:8,flexWrap:'wrap',marginBottom:4}}>
          {[0,1,2,3,4,5,8,10].map(n=>(
            <TouchableOpacity key={n} style={{paddingHorizontal:14,paddingVertical:8,borderRadius:16,borderWidth:1.5,borderColor:guestCount===n?'#1B3A5C':'#D4EDE5',backgroundColor:guestCount===n?'#1B3A5C':'rgba(255,255,255,0.9)'}} onPress={()=>setGuestCount(n)}>
        ...
        {guestCount > 0 && (
          <View style={{marginTop:8}}>
            <Text style={{fontSize:11,color:'#5A7A8A',marginBottom:6}}>GUEST CUISINE PREFERENCE</Text>
            <View style={{flexDirection:'row',gap:8,flexWrap:'wrap'}}>
              {GUEST_CUISINES.map(c=>(
                <TouchableOpacity key={c} style={{paddingHorizontal:12,paddingVertical:7,borderRadius:20,borderWidth:1.5,borderColor:guestCuisine===c?'#C9A227':'#D1D5DB',backgroundColor:guestCuisine===c?'#C9A227':'rgba(255,255,255,0.9)'}} onPress={()=>setGuestCuisine(guestCuisine===c?'':c)} activeOpacity={0.8}>
                  <Text style={{fontSize:12,fontWeight:'600',color:guestCuisine===c?'#1B2A0C':'#1B3A5C'}}>{c}</Text>
                </TouchableOpacity>
              ))}
            </View>
          </View>
        ...
        <View style={s.btnRow}>
          ...
          <View style={{ flex: 2 }}>
            <Button title="Next" onPress={() => {
              ...
              if ((foodPref === 'nonveg' || isMixed) && nonVegOpts.length === 0) { setError('Please select at least one protein'); return; }
              advance('meal-prefs');
            }} />
          </View>
        </View>
  ...
  function renderNutrition() {
    const moods: { key: typeof weekMood; title: string; desc: string }[] = [
      { key: 'balanced', title: 'Balanced', desc: 'Nutritious mix, all food groups' },
      { key: 'light',    title: 'Light',    desc: 'Easy on the stomach, simple dishes' },
      { key: 'cheat',    title: 'Cheat Day', desc: 'Indulgent, rich, comfort food' },
      { key: 'festival', title: 'Festival',  desc: 'Traditional festive dishes and sweets' },
    ];
    return (
      <View>
        <Text style={s.stepTitle}>What mood this week?</Text>
        <Text style={s.stepSub}>Maharaj will tailor every meal to match</Text>
        <View style={{flexDirection:'row',flexWrap:'wrap',gap:12}}>
          {moods.map(m => (
            <TouchableOpacity key={m.key} style={{width:'47%',backgroundColor:weekMood===m.key?'rgba(27,58,92,0.06)':'rgba(255,255,255,0.95)',borderRadius:14,padding:16,borderWidth:2,borderColor:weekMood===m.key?navy:'#E5E7EB'}} onPress={()=>setWeekMood(m.key)} activeOpacity={0.8}>
              <Text style={{fontSize:16,fontWeight:'700',color:navy,marginBottom:4}}>{m.title}</Text>
              <Text style={{fontSize:12,color:'#5A7A8A',lineHeight:18}}>{m.desc}</Text>
            </TouchableOpacity>
          ))}
        </View>
  ...
  function renderSelection() {
    ...
    return (
      <View style={{flex:1,alignItems:'center'}}>
        ...
        {visibleSlots.map(({ key, label }) => {
          ...
          return (
            <View key={key} style={{marginBottom:12,width:'100%',maxWidth:600,alignSelf:'center'}}>
              {/* Slot header bar */}
              <View style={{backgroundColor:navy,borderRadius:10,paddingHorizontal:14,paddingVertical:8,marginBottom:8,flexDirection:'row',alignItems:'center',justifyContent:'space-between'}}>
                <Text style={{fontSize:14,fontWeight:'700',color:white}}>{label}</Text>
                {selections[activeDay]?.[key] !== undefined && (
                  <View style={{backgroundColor:'#C9A227',borderRadius:10,paddingHorizontal:8,paddingVertical:3}}>
                    <Text style={{fontSize:10,fontWeight:'600',color:'#1B2A0C'}} numberOfLines={1}>{slotData.options[selections[activeDay][key]]?.name?.split(' ').slice(0,3).join(' ')}</Text>
                  </View>
                )}
              </View>
              {slotData.options.map((opt, optIdx) => {
                ...
                return (
                  ...
                  >
                    ...
                    {isSel && (
                      <View style={{marginTop:6,marginLeft:32}}>
                        {hasPipe ? (
                          ...
                          <Text style={{fontSize:12,color:'#374151',lineHeight:20}}>{opt.description}</Text>
                        ) : opt.ingredients && opt.ingredients.length > 0 ? (
                          <>
                            <Text style={{fontSize:11,fontWeight:'700',color:'#1B3A5C',marginBottom:3}}>Ingredients:</Text>
                            <Text style={{fontSize:11,color:'#374151',lineHeight:18}}>{opt.ingredients.slice(0,6).join(', ')}</Text>
                          </>
                        ) : null}
                      </View>
                    )}
                    ...
                    {!isSel && summaryLine && (
                      <Text style={{fontSize:12,color:'#9CA3AF',marginTop:2,marginLeft:32,lineHeight:18}} numberOfLines={2}>{summaryLine}</Text>
                    )}
                    {/* Tags */}
                    {opt.tags.length > 0 && (
        ...
        {/* Summary bar */}
        <View style={{backgroundColor:'rgba(255,255,255,0.95)',borderTopWidth:0.5,borderTopColor:'rgba(27,58,92,0.1)',padding:12,width:'100%',maxWidth:600,alignSelf:'center'}}>
          <Text style={{fontSize:10,color:'#6B7280',marginBottom:6}}>Your selections</Text>
          <View style={{flexDirection:'row',flexWrap:'wrap',gap:5,marginBottom:10}}>
            {visibleSlots.map(({key,label})=>{
              const selIdx = selections[activeDay]?.[key];
              const selName = selIdx !== undefined ? generatedPlan[activeDay]?.[key]?.options[selIdx]?.name : null;
              return (
                <View key={key} style={{backgroundColor:selName?'#E8F5F0':'transparent',borderWidth:selName?0:1,borderStyle:selName?'solid':'dashed' as any,borderColor:selName?'transparent':'#D1D5DB',borderRadius:20,paddingHorizontal:10,paddingVertical:4}}>
                  <Text style={{fontSize:10,fontWeight:'500',color:selName?'#0F6E56':'#9CA3AF'}}>{selName?selName.split(' ').slice(0,3).join(' '):`${label}...`}</Text>
                </View>
              );
            })}
          </View>
          ...
          <View style={{flexDirection:'row',gap:8,marginTop:8}}>
            ...
            <TouchableOpacity style={{flex:1,paddingVertical:11,borderRadius:12,borderWidth:1.5,borderColor:'#D4EDE5',backgroundColor:'rgba(255,255,255,0.9)',alignItems:'center'}} onPress={()=>router.push('/home' as never)}>
    ...
    return (
      <View>
        ...
        <View style={{flexDirection:'row',gap:8,flexWrap:'wrap',marginBottom:12}}>
          <TouchableOpacity style={{paddingHorizontal:16,paddingVertical:9,borderRadius:20,borderWidth:1.5,borderColor:!hasGuests?'#1B3A5C':'#D1D5DB',backgroundColor:!hasGuests?'#1B3A5C':'rgba(255,255,255,0.9)'}} onPress={()=>{setHasGuests(false);setGuestCuisine('');}}>
            <Text style={{fontSize:13,fontWeight:'600',color:!hasGuests?white:'#6B7280'}}>Just my family</Text>
          </TouchableOpacity>
          <TouchableOpacity style={{paddingHorizontal:16,paddingVertical:9,borderRadius:20,borderWidth:1.5,borderColor:hasGuests?'#1B3A5C':'#D1D5DB',backgroundColor:hasGuests?'#1B3A5C':'rgba(255,255,255,0.9)'}} onPress={()=>setHasGuests(true)}>
            <Text style={{fontSize:13,fontWeight:'600',color:hasGuests?white:'#6B7280'}}>We have guests</Text>
          </TouchableOpacity>
        </View>
        {hasGuests && (<View>
      ...
      <View style={{maxWidth:794,width:'100%',alignSelf:'center'}}>
            ...
            {slotsToShow.map(({ key, label }, slotIdx) => (
              <View key={key} style={{flexDirection:'row',borderTopWidth:1,borderTopColor:BORDER}}>
                ...
                {generatedPlan.map((day, dayIdx) => {
                  const skipped = isSkipped(dayIdx, key);
                  const opt = skipped ? null : getOpt(dayIdx, key);
        ...
        <View style={{flexDirection:'row',gap:8}}>
          <TouchableOpacity style={{flex:1,paddingVertical:14,borderRadius:12,borderWidth:1.5,borderColor:'rgba(27,58,92,0.3)',backgroundColor:'rgba(255,255,255,0.9)',alignItems:'center'}} onPress={async()=>{if(await tryRegenerate()){setGeneratedPlan(null);setSelections({});setActiveDay(0);setStep('generating');}}}>
            <Text style={{fontSize:13,fontWeight:'600',color:'#1B3A5C'}}>Regenerate</Text>
            <Text style={{fontSize:10,color:'#9CA3AF',marginTop:2}}>{regenCount}/5 today</Text>
          </TouchableOpacity>
          <TouchableOpacity style={{flex:1,paddingVertical:14,borderRadius:12,borderWidth:1.5,borderColor:navy,backgroundColor:white,alignItems:'center'}} onPress={()=>{setGeneratedPlan(null);setSelections({});setActiveDay(0);setSelectedFrom(null);setSelectedTo(null);setFoodPref(null);setIsMixed(false);setSelectedSlots([]);setGuestCount(0);setHasGuests(false);setGuestCuisine('');setBfPrefs([]);setLnPrefs([]);setDnPrefs([]);setSnPrefs([]);setUnwellIds([]);setEveryoneWell(true);setVegFastDays({});setNutritionGoals([]);setRemovedCuisines([]);setExtraCuisines([]);setPerDayCuisine({});setIncludeDessert(false);setFeedbacks([]);setFeedbackDone(false);setSkippedSlots({});session.presentMemberIds=[];setStep('period');}}>
            <Text style={{fontSize:13,fontWeight:'600',color:navy}}>Modify</Text>
          </TouchableOpacity>
          <TouchableOpacity style={{flex:2,paddingVertical:14,borderRadius:12,backgroundColor:navy,alignItems:'center'}} onPress={()=>advance('recipes')}>
            <Text style={{fontSize:14,fontWeight:'700',color:white}}>View Recipes</Text>
          </TouchableOpacity>
        </View>
        <TouchableOpacity style={{paddingVertical:10,alignItems:'center',marginTop:4}} onPress={()=>setStep('confirmed-menu')}>
          <Text style={{fontSize:13,fontWeight:'600',color:'#5A7A8A'}}>Back to Plan</Text>
        </TouchableOpacity>
      </View>
    );
  }
      ...
      <View>
        ...
        <View style={{gap:12,marginTop:8}}>
          <TouchableOpacity style={{backgroundColor:'#1B3A5C',borderRadius:14,paddingVertical:18,alignItems:'center'}} onPress={() => advance('generating')} activeOpacity={0.85}>
            <Text style={{fontSize:16,fontWeight:'800',color:'white'}}>Plan with Maharaj</Text>
          </TouchableOpacity>
          <View style={{flexDirection:'row',gap:10}}>
