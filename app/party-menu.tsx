  ...
  async function saveToHistory(m: PartyMenu) {
    try {
      ...
      await supabase.from('menu_history').insert({
        ...
        menu_json: { type: 'party', occasion: occasionText, recognised: recogniseOccasion(occasionText), guests: guestCountText, budget, foodType, starters: m.starters, main_course: m.main_course, desserts: m.desserts, beverages: m.beverages, serving_tips: m.serving_tips, shopping_list: m.shopping_list },
      });
      setSaved(true);
    } catch (e) { console.error('[PartyMenu] save error', e); }
  }
